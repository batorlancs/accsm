...
export function AccSetupManager() {
  ...
  const [viewState, setViewState] = useState<MainViewState>({ type: 'empty' });
  const [isPathDialogOpen, setIsPathDialogOpen] = useState(false);

  const handleSelectSetup = (car: string, track: string, filename: string) => {
  ...
  const selectedSetup = viewState.type === 'viewing' 
    ? { car: viewState.car, track: viewState.track, filename: viewState.filename }
    : null;

  return (
    <div className="h-screen flex flex-col bg-background">
      {/* Header */}
      <div data-tauri-drag-region className="h-12 w-full bg-primary text-primary-foreground flex items-center px-4 justify-between">
