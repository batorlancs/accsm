...
export function FileDropModal({ open, onOpenChange, globalDropFiles }: FileDropModalProps) {
    const [dropState, setDropState] = useState<DropState>({ type: "waiting" });
    const queryClient = useQueryClient();
    const processedGlobalFiles = useRef<string | null>(null);

    const onSuccess = useCallback((results: any) => {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Drop JSON setup files or folders containing JSON files to import them into your setup collection.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4">
                    {dropState.type === "waiting" && (
                        <>
                            <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center space-y-4">
                                ...
                                <div>
                                    <h3 className="font-semibold mb-2">Drop files or folders here</h3>
                    ...
                    {dropState.type === "dragover" && (
                        <div className="border-2 border-dashed border-primary rounded-lg p-8 text-center space-y-4 bg-primary/5">
                            ...
                            <div>
                                <h3 className="font-semibold mb-2 text-primary">Release to import</h3>
                                <p className="text-sm text-muted-foreground">
                    ...
                    {dropState.type === "processing" && (
                        <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center space-y-4">
                            ...
                            <div>
                                <h3 className="font-semibold mb-2">Processing files...</h3>
                                <p className="text-sm text-muted-foreground">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-4 text-sm">
                                    <span className="flex items-center gap-1 text-green-600">
                                        ...
                                        {getSuccessCount(dropState.results)} successful
                                    </span>
                                    {getFailureCount(dropState.results) > 0 && (
                                        <span className="flex items-center gap-1 text-red-600">
                                            <AlertCircle className="h-4 w-4" />
                                            {getFailureCount(dropState.results)} failed
                                        </span>
                                    )}
                                </div>
                            </div>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {result.success ? (
                                                <p className="text-xs text-green-700 dark:text-green-300">
                                                    Imported as {result.car}/{result.track}/{result.filename}
                                                </p>
                                            ) : (
