...
export const queryKeys = {
  ...
  setup: (car: string, track: string, filename: string) => ['setup', car, track, filename] as const,
  cars: ['cars'] as const,
...
// Setup queries
export function useSetup(car: string, track: string, filename: string, enabled = true) {
  return useQuery({
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.setup(variables.car, variables.track, variables.filename),
      });
      queryClient.invalidateQueries({
        queryKey: queryKeys.folderStructure,
      });
...
export function useValidateSetup() {
  return useMutation({
    mutationFn: (params: ValidateSetupParams) => TauriAPI.validateSetup(params),
    onError: (error) => {
